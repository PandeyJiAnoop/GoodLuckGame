...
import com.akp.aonestar.New_ShopIt.ShopIt_BasicPackage;
import com.akp.aonestar.New_ShopIt.ShopIt_PrimePackageList;
import com.akp.aonestar.New_ShopIt.ShopIt_productBetting;
...
public class GoodLuckGameProductView extends AppCompatActivity {
    LinearLayout ll;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_good_luck_game_product_view);
        ...
        mp = MediaPlayer.create(GoodLuckGameProductView.this, R.raw.akp_click);
        ...
        if (UserId.equalsIgnoreCase("")) {
            ...
            welcome_name_tv.setVisibility(View.GONE);
        } else {
            welcome_name_tv.setVisibility(View.VISIBLE);
            text_ll.setVisibility(View.GONE);
            welcome_name_tv.setText("Welcome to ShopIt\n" + UserName + "(" + UserId + ")");
            welcome_name_tv.setTextColor(Color.parseColor("#E8C326"));
            buynow_btn.setVisibility(View.INVISIBLE);
        }
...
//        buynow_btn.startAnimation(an);
        details.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) { GameDetails_popup1("GoodLuck");
            }
        });
...
//                 Toast.makeText(getApplicationContext(),""+response,Toast.LENGTH_LONG).show();
                ...
                try {
                    ...
                    if (status.equalsIgnoreCase("true")) {
                        ...
                        Toast.makeText(GoodLuckGameProductView.this, object.getString("Message"), Toast.LENGTH_SHORT).show();
                        AlertDialog.Builder builder = new AlertDialog.Builder(GoodLuckGameProductView.this);
            ...
            public void onErrorResponse(VolleyError error) {
                progressDialog1.dismiss();
                Toast.makeText(GoodLuckGameProductView.this, "Something went wrong:-" + error, Toast.LENGTH_SHORT).show();
            }
        }) {
            @Override
        ...
        RequestQueue requestQueue = Volley.newRequestQueue(GoodLuckGameProductView.this);
                ...
                try {
                    ...
                    if (status.equalsIgnoreCase("true")) {
                        ...
                        for (int i = 0; i < Response.length(); i++) {
                            ...
                            Glide.with(GoodLuckGameProductView.this).load(jsonObject.getString("BannerImg")).error(R.drawable.a_logo).into(dumy);
                        }
                    } else {
                        Toast.makeText(GoodLuckGameProductView.this, object.getString("Message"), Toast.LENGTH_SHORT).show();
                    }
...
//                 Toast.makeText(getApplicationContext(),""+response,Toast.LENGTH_LONG).show();
    ...
    public void CategoryAPI() {
        final ProgressDialog progressDialog = new ProgressDialog(GoodLuckGameProductView.this);
        progressDialog.setMessage("Loading");
        progressDialog.show();
        StringRequest stringRequest = new StringRequest(Request.Method.GET, AppUrls.baseUrl + "GameCategoryList", new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
...
//                            title_tv.setText("External Link Report("+Response.length()+")");
                        ...
                        Toast.makeText(GoodLuckGameProductView.this, "No data found", Toast.LENGTH_SHORT).show();
                    }
                } catch (JSONException e) {
                    e.printStackTrace();
                }
    ...
    public class FriendsListAdapter extends RecyclerView.Adapter<FriendsList> {
        ...
        public FriendsList onCreateViewHolder(ViewGroup parent, int viewType) {
            return new FriendsList(LayoutInflater.from(parent.getContext()).inflate(R.layout.dynamic_catshopit, parent, false));
        }

        @SuppressLint("SetTextI18n")
        public void onBindViewHolder(final FriendsList holder, final int position) {
            holder.cat_name.setText(data.get(position).get("CatName"));
            if (data.get(position).get("CatImg").equalsIgnoreCase("")) {
                Toast.makeText(getApplicationContext(), "Image not found!", Toast.LENGTH_LONG).show();
            } else {
                Glide.with(GoodLuckGameProductView.this).load(data.get(position).get("CatImg")).into(holder.imgb);
            }

            holder.itemView.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    Log.d("res_id",data.get(position).get("PKID"));
                    mp.start();
                    if (data.get(position).get("PKID").equalsIgnoreCase("1")) {
                        Intent intent = new Intent(getApplicationContext(), FreeJoiningProduct.class);
                        startActivity(intent);
                    } else if (data.get(position).get("PKID").equalsIgnoreCase("3")) {
                        Intent intent = new Intent(getApplicationContext(), ShopIt_PrimePackageList.class);
                        intent.putExtra("cat_id",data.get(position).get("PKID"));
                        startActivity(intent);
//                        Intent intent = new Intent(getApplicationContext(), ConsolationProduct.class);
//                        startActivity(intent);
                    } else if (data.get(position).get("PKID").equalsIgnoreCase("7")) {
                        Intent intent = new Intent(getApplicationContext(), ShopIt_BasicPackage.class);
                        intent.putExtra("cat_id",data.get(position).get("PKID"));
                        startActivity(intent);
//                        Intent intent = new Intent(getApplicationContext(), ConsolationNew.class);
//                        startActivity(intent);
                    ...
                    startActivity(intent);
                        ...
                        intent.putExtra("update_name",data.get(position).get("CatName"));
                        startActivity(intent);
                    }}});
    ...
    private void GameDetails_popup1(String getId1) {
        ...
        View dialogView = LayoutInflater.from(GoodLuckGameProductView.this).inflate(R.layout.popup_gameoverview, viewGroup, false);
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.POST, AppUrls.baseUrl + "MainGameDetailsbyCat", new Response.Listener<String>() {
            ...
            public void onResponse(String response) {
                ...
                try {
                    ...
                    Toast.makeText(GoodLuckGameProductView.this, "No Record Found!", Toast.LENGTH_SHORT).show();
                    e.printStackTrace();
                }
            }
        }, new Response.ErrorListener() {
...
//                Toast.makeText(GoodLuckGameProductView.this, "No Record Found!", Toast.LENGTH_SHORT).show();
        ...
        AlertDialog.Builder builder1 = new AlertDialog.Builder(GoodLuckGameProductView.this);
